...
const ManageBilling = () => {
	...
	const fetchPlan = async (planId: string) => {
		try {
			const { data, error } = await getPlanDetails(planId);
	...
	useEffect(() => {
		let planId = '';
		if (typeof userData?.subscription?.planId === 'object') {
			planId = userData?.subscription?.planId?._id;
		} else {
		...
		<div className="Billing max-w-4xl mx-auto  sm:mt-24  sm:mb-24 py-5 sm:py-0 mb-20 relative px-4">
			<div className="mb-12">
				<h2 className="sm:text-5xl text-[24px] font-PoppinsBold text-111 flex items-center mb-8 mt-10">
					...
					<div className="ml-10">Enter your billing information</div>
				</h2>
			...
			<div className="mx-auto grid md:grid-cols-6 grid-flow-row md:grid-flow-col gap-4">
				...
				<div
					...
					{/* <h2 className="text-xl text-center md:text-left md:text-4xl font-PoppinsBold text-111 mb-8"></h2> */}
